...
void JetImageData::waitForNext(){
	while(true){
	  //wait for condition
	  {
	    ...
	    edm::LogInfo("TFClientRemoteTRT") << " -->  time to fill image array " << time2 << " -- " << std::endl;
	    ...
	    edm::LogInfo("TFClientRemoteTRT") << " -->  time to run " << time3 << " -- " << std::endl;
	    ...
	    edm::LogInfo("TFClientRemoteTRT") << " -->  time to output " << time4 << " -- " << std::endl;
...
{ 
  ...
  input->Reset();
  nicinput_ = &input;
  std::unique_ptr<nic::ServerHealthContext> context;
  nic::Error err = nic::ServerHealthGrpcContext::Create(&context, url, true);
  bool live, ready;
  err = context->GetLive(&live);
  err = context->GetReady(&ready);
  */
}
...
void TFClientRemoteTRT::predict(unsigned dataID, const float* img, float* result, edm::WaitingTaskWithArenaHolder holder) {
	...
	//do all read/writes inside lock to ensure cache synchronization
	{
	  ...
	  //streamData.context_  = context_;
	  //streamData.nicinput_ = nicinput_;
